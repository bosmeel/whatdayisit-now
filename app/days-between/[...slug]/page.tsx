...
function parseSlug(slug: any) {
  const text = Array.isArray(slug) ? slug.join("-") : slug;
  if (!text) return null;
...
export default function Page({ params }: any) {
  ...
  if (!parsed) {
    return (
      <main className="max-w-2xl mx-auto py-12">
        ...
        <p className="mb-6">Example: january-1-and-december-31</p>
  ...
  const label1 = date1.toLocaleDateString("en-US", { month: "long", day: "numeric" });
  const label2 = date2.toLocaleDateString("en-US", { month: "long", day: "numeric" });
  ...
  return (
    <main className="max-w-2xl mx-auto py-12">
      ...
      <p className="text-3xl font-semibold mb-8">{diff} days</p>
